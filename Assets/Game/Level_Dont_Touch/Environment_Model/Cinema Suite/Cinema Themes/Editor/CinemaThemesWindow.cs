...
public class CinemaThemesWindow : EditorWindow
{
...
#endif
    private const string MISSING_LUTS_ERROR_MSG =
        "No LUTs were found in the project! \nPlease re-import the Cinema Themes package.";

    private const string MISSING_CAMERA_ERROR_MSG =
    ...
    protected void OnGUI()
    {
        ...
        bool isComponentAttached =
            cameraExists ? isColorCorrectionAttached(Camera.allCameras[selectedCameraIndex]) : false;
        ...
        EditorGUI.BeginDisabledGroup(!cameraExists || AssetDatabase.GetAssetPath(texture) ==
            getTempTexturePath(Camera.allCameras[selectedCameraIndex]));
    ...
    protected void LoadLUTs()
    {
        ...
        {
            Texture2D t2d =
                AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(guid), typeof(Texture2D)) as Texture2D;
            luts.Add(t2d);
        }
